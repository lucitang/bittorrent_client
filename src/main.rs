...
async fn main() -> Result<(), Error> {
    ...
    match args.subcmd {
        ...
        Commands::Download {
            ...
            let mut peer = Peer::from(&peers[2].address);
            peer.handshake(&info_hash, &peer_id);

            // Make sur the peer is ready to receive requests
            check_readiness(&mut peer);

            let mut pieces: Vec<u8> = vec![];
            for piece_index in 0..torrent.info.pieces.chunks(20).len() {
                if let Ok(piece_data) = download_piece(piece_index as i32, &torrent, &mut peer) {
...
fn check_readiness(peer: &mut Peer) {
    println!("–––––––––––––––––––––––––––––––––––––");
...
fn download_piece(piece_index: i32, torrent: &Torrent, peer: &mut Peer) -> Result<Vec<u8>, Error> {
